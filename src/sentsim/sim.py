...
class Embedder(object):
    def __init__(self, path: Path) -> None:
        if not path:
            path = Path(__file__).parent.parent / 'assets'/'word2vec'/'GoogleNews-vectors-negative300.bin'
...
def set_assets(assets: Path):
    embedder = Embedder.instance( path=Path(assets) / 'word2vec' / 'GoogleNews-vectors-negative300.bin')
